...
class OfficeDataset(Dataset):
    def __init__(self, base_path, site, train=True, transform=None):
        if train:
            self.paths, self.text_labels = np.load(os.path.join(base_path, 'office_caltech_10', '{}_train.pkl'.format(site)), allow_pickle=True)
        else:
            self.paths, self.text_labels = np.load(os.path.join(base_path, 'office_caltech_10', '{}_train.pkl'.format(site)), allow_pickle=True)
...
class DomainNetDataset(Dataset):
    def __init__(self, base_path, site, train=True, transform=None):
        ...
        if train:
            self.paths, self.text_labels = np.load(os.path.join(base_path, 'DomainNet', '{}_train.pkl'.format(site)), allow_pickle=True)
        else:
            self.paths, self.text_labels = np.load(os.path.join(base_path, 'DomainNet', '{}_test.pkl'.format(site)), allow_pickle=True)
